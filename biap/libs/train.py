...
def train(
    ...
    with open('results_copy.txt', 'a') as file_copy:
        file_copy.write(('%8s%12s' + '%10s' * 8 + '%11s' * 6 + '\n') % (
            'Epoch', 'Batch', 'xy', 'wh', 'conf', 'cls', 'total', 'targets', 'time', 'loss',
            'P', 'R', 'mAP', 'F1', 'Test_Loss', 'maps'))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Fre   eze backbone at epoch 0, unfreeze at epoch 1 (optional)
        ...
        for i, (imgs, targets, _, _) in enumerate(dataloader):
            ...
            print('loss_items:{}'.format(loss_items))
            ...
            s = (('%8s%12s' + '%10.3g' * 8) % (
                '%g/%g' % (epoch, epochs - 1),
                '%g/%g' % (i, nb - 1), *mloss, len(targets), time.time() - t, loss))
            table = [[('\n%8s%12s' + '%10s' * 8) % (
                'Epoch', 'Batch', 'xy', 'wh', 'conf', 'cls', 'total', 'targets', 'time', 'loss')], [s]]
            # table_data = [['Epoch', 'Batch', 'xy', 'wh', 'conf', 'cls', 'total', 'targets', 'time', 'loss', 'loss_items']]
            table_instance = AsciiTable(table).table
        ...
        with open('results.txt', 'a') as file:
            file.write(s + '%11.3g' * 5 % results + '%11.3g' % maps + '\n')  # P, R, mAP, F1, test_loss(per epoch)，maps

        with open('results_copy.txt', 'a') as file_copy:
            file_copy.write(s + '%11.3g' * 5 % results + '%11.3g' % maps + '\n')
        ...
        #
        #     if epoch % 2 == 0:
        #         print("train_epoch:{}".format(epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-cfg', type=str, default='data/head.data', help='person.data file path')
